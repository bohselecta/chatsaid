// Fixed AI simulation script - uses existing user account
...
async function seedWeekActivity() {
  ...
  console.log('\n🔍 Getting your user account...');
  const { data: { user }, error: userError } = await supabase.auth.getUser();
  
  if (userError || !user) {
    console.error('❌ Could not get user. Please log in to ChatSaid first.');
    return;
  }
    ...
    for (const bot of bots) {
      ...
      for (let i = 0; i < cherriesPerDay; i++) {
        try {
          ...
          // Insert cherry using YOUR user ID
            ...
            .insert([
              {
                ...
                tags: cherry.tags,
                author_id: user.id, // Use your real user ID
...
Output as JSON: { content }
          ...
          for (let reactionIndex = 0; reactionIndex < numReactions; reactionIndex++) {
            ...
            const { error: reactionErr } = await supabase.from('user_reactions').insert([
              {
                cherry_id: cherryData[0].id,
                user_id: user.id, // Use your real user ID
