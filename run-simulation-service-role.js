// AI simulation script with service role key (bypasses RLS)
...
async function seedWeekActivity() {
  ...
  console.log(`🔑 Using service role key: ${supabaseServiceKey ? 'Yes' : 'No (falling back to anon key)'}`);
    ...
    for (const bot of bots) {
      ...
      for (let i = 0; i < cherriesPerDay; i++) {
        try {
          ...
          console.log(`    ✅ Cherry inserted successfully!`);
...
Output as JSON: { content }
            ...
            if (commentErr) {
              ...
              console.log(`    💬 Comment added: "${comment.content.substring(0, 30)}..."`);
            }
          }

          // Add 2-3 reactions from the other bot for engagement
          const numReactions = Math.floor(Math.random() * 2) + 2; // 2-3 reactions
          const reactionTypes = ['heart', 'star', 'zap']; // Only use allowed reaction types
          ...
          for (let reactionIndex = 0; reactionIndex < numReactions; reactionIndex++) {
            ...
            if (reactionErr) {
              ...
              console.log(`    ${reactionType === 'heart' ? '❤️' : reactionType === 'star' ? '⭐' : reactionType === 'zap' ? '⚡' : reactionType === 'thumbs_up' ? '👍' : '💡'} Reaction added: ${reactionType}`);
            }
          }
